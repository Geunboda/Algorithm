...
//		차번호로 정렬에서 차이름으로 변경 할 경우. 또는 오름차순을 내림차순으로 변경할 경우. compareTo() 변경 불가피하다.
//		바뀌지 않는 기준을 이용해서 정렬할 경우 사용한다.
		System.out.println("\n[차번호로 내림차순 정렬]");
		Collections.sort(list);
		view(list);
		//---------------------------------------------------------------------------------------		
//		Comparator는 특정한 데이터를 그때그때 정렬하는 기준을 정해야 하는경우
//		compareTo()를 매번 변경 할 수 없으므로 단발성이나 다양한 기준을 세우고 싶은 경우 사용.
		System.out.println("\n[차번호로 오름차순 정렬]");
		
		Comparator<Car> carNumComparator = new Comparator<Car>() {
		...
		Collections.sort(list, carNumComparator);
		view(list);
		//-----------------------------------------------------------------------------------------		
		System.out.println("\n[제조사로 내림차순 정렬]");		
